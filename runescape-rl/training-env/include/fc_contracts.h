...
#define FC_NUM_ACTION_HEADS     5

/* Head 0: MOVE — directional tile movement */
/*
 *   0     = idle (no movement)
 *   1-8   = walk 1 tile (N, NE, E, SE, S, SW, W, NW)
 *   9-16  = run 2 tiles (N, NE, E, SE, S, SW, W, NW)
 *
 * Click-to-move in the viewer: pathfind from player position to clicked tile,
 * then enqueue one MOVE direction per tick until destination reached.
 * The viewer does NOT have a hidden "walk to (x,y)" API.
 */
...
#define FC_ACT_SIZES { FC_MOVE_DIM, FC_ATTACK_DIM, FC_PRAYER_DIM, FC_EAT_DIM, FC_DRINK_DIM }
 ...
 *
 * Layout: [MOVE mask (17)] [ATTACK mask (9)] [PRAYER mask (5)] [EAT mask (3)] [DRINK mask (2)]
 */
#define FC_ACTION_MASK_SIZE     (FC_MOVE_DIM + FC_ATTACK_DIM + FC_PRAYER_DIM + FC_EAT_DIM + FC_DRINK_DIM)  /* 36 */
 ...
 *   FC_POLICY_OBS_SIZE (126) + FC_REWARD_FEATURES (16) + FC_ACTION_MASK_SIZE (36) = 178
 *
 * PufferLib sees OBS_SIZE = 178.
 * Python trainer slices:
 *   policy_obs   = obs[:126]
 *   reward_feat  = obs[126:142]
 *   action_mask  = obs[142:178]
 */
