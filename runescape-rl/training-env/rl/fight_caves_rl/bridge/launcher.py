...
HEADLESS_EXTRACT_ROOT = runtime_subdir("artifacts", "training-env", "rl", "headless_dist")
...
def _extract_distribution(distribution_zip: Path, extract_root: Path) -> Path:
    ...
    lock_dir = target_root.parent / f".{distribution_zip.stem}.extract-lock"
    ...
    acquired_lock = _acquire_extract_lock(lock_dir, marker=marker, distribution_zip=distribution_zip)
    if not acquired_lock:
        extracted = _resolve_distribution_root(target_root)
        if extracted is not None:
            return extracted
        raise SimArtifactNotFoundError(
            "Timed out waiting for another process to extract the headless distribution: "
            f"{distribution_zip}"
        )
    ...
    try:
        ...
        _release_extract_lock(lock_dir)
...
def _release_extract_lock(lock_dir: Path) -> None:
    try:
        os.rmdir(lock_dir)
    except FileNotFoundError:
        pass
    except OSError:
        # If another process already replaced or removed the lock, ignore.
        pass
