...
class ToolLeprechaun : Script {
    ...
    init {
        ...
        itemOnNPCOperate("*", "tool_leprechaun*") {
            ...
            if (noted == null) {
                npc<Neutral>("Nay, there's no such thing as a banknote for that.")
                return@itemOnNPCOperate
            }
            if (!item.def["compostable", false]) {
                npc<Neutral>("Nay, I've got no banknotes to exchange for that item.")
                return@itemOnNPCOperate
            }
            var removed = 0
    ...
    private fun ChoiceOption.noThanks() {
        option<Neutral>("No thanks, I'll keep hold of my stuff.") {
            npc<Laugh>("Ye must be dafter than ye look if ye likes luggin' yer tools everywhere ye goes!")
        }
    }
