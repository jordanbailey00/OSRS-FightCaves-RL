...
object Main {
    ...
    fun main(args: Array<String>) {
        start(GameProfiles.default)
    }

    fun start(profile: GameProfile = GameProfiles.default) {
        AuditLog.info("startup")
        AuditLog.info("profile ${profile.id}")
        val startTime = System.currentTimeMillis()
        val settings = settings(profile)
    ...
    private fun settings(profile: GameProfile): Properties = timed("properties") {
        Settings.clear()
        var properties: Properties? = null
        for (fileName in profile.propertyFiles) {
            properties = Settings.load(fileName)
        }
        val loaded = properties ?: Settings.load(DEFAULT_PROPERTY_FILE)
        loaded.putAll(System.getenv())
        return@timed loaded
    }
