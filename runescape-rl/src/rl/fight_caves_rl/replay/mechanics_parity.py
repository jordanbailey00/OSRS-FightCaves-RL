...
HEADLESS_PARITY_RUNTIME_PATH_ID = "headless"
SUPPORTED_SCENARIO_TRACE_PATHS = (ORACLE_RUNTIME_PATH_ID, HEADLESS_PARITY_RUNTIME_PATH_ID)
...
def collect_named_parity_scenario_trace(
    runtime_path: str,
    scenario_id: str,
) -> dict[str, Any]:
    if runtime_path not in SUPPORTED_SCENARIO_TRACE_PATHS:
        raise ValueError(
            f"Unsupported named scenario runtime path: {runtime_path!r}. "
            f"Expected one of {SUPPORTED_SCENARIO_TRACE_PATHS!r}."
        )
    paths = discover_headless_runtime_paths()
    assert_sim_runtime_ready(paths)
    classes = _ensure_fast_jvm(paths.headless_jar, paths.launch_cwd)
    ...
    for key, value in _parity_harness_settings_overrides(paths).items():
        overrides.put(str(key), str(value))
    parity_harness = classes["jpype"].JClass("ParityHarness")()
    traces = parity_harness.collectScenarioMechanicsParityTrace(
        str(runtime_path),
        str(scenario_id),
        overrides,
    )
    records = adapt_fast_parity_traces(traces)
    return _build_named_trace_export_payload(
...
def _build_named_trace_export_payload(
    *,
    runtime_path: str,
    scenario_id: str,
    records: Sequence[Mapping[str, Any]],
