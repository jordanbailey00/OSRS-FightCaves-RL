...
class ConfigurablePuffeRL(pufferlib.pufferl.PuffeRL):
    ...
    def evaluate(self):
        ...
        # (sample_logits_gumbel) is used instead of PufferLib's base evaluate
        # which falls back to torch.multinomial.  Instrumentation timing calls
        # are guarded by _record_instrumentation (no-op when disabled).
    ...
    @pufferlib.pufferl.record
    def train(self):
        # Always use the custom train loop so that Gumbel-max log-prob
        # recomputation and fused entropy (T2.1) are used instead of
        # PufferLib's base train which falls back to torch.multinomial
        # and unfused entropy.
        profile = self.profile
        epoch = self.epoch
        profile("train", epoch)
        ...
        for mb in range(self.total_minibatches):
            ...
            if a == 0:
                # T2.2: skip priority weighting when prio_alpha=0.
                # Uniform random selection — mathematically equivalent to
                # multinomial on uniform probs, but avoids the adv**0 / softmax / multinomial chain.
                idx = torch.randperm(self.segments, device=device)[:self.minibatch_segments]
