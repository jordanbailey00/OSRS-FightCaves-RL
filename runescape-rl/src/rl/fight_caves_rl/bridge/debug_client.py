    ...
    def reset_episode(self, player: Any, config: HeadlessEpisodeConfig) -> dict[str, Any]:
        ...
        return {
            ...
            "player_tile": self._tile_from_id(
                _invoke_jvm_getter(state, "getPlayerTile-aANnmZU", "getPlayerTile")
            ),
        }
            ...
            targets.append(
                {
                    ...
                    "tile": self._tile_from_id(
                        _invoke_jvm_getter(target, "getTile-aANnmZU", "getTile")
                    ),
                }
            )
        return targets
...
def _ensure_jvm(headless_jar: Path, launch_cwd: Path) -> dict[str, Any]:
    ...
    classes = {
        ...
        "BodyParts": jpype.JClass("sim.engine.entity.character.player.equip.BodyParts"),
        "ExchangeOffer": jpype.JClass("sim.engine.data.exchange.ExchangeOffer"),
        "ExchangeOfferArray": jpype.JArray(jpype.JClass("sim.engine.data.exchange.ExchangeOffer")),
        "Experience": jpype.JClass("sim.engine.entity.character.player.skill.exp.Experience"),
        ...
        "Inventories": jpype.JClass("sim.engine.inv.Inventories"),
        "JvmClassMappingKt": jpype.JClass("kotlin.jvm.JvmClassMappingKt"),
        "KoinKt": jpype.JClass("sim.engine.KoinKt"),
        "Levels": jpype.JClass("sim.engine.entity.character.player.skill.level.Levels"),
        "AccountManager": jpype.JClass("sim.engine.data.AccountManager"),
        "Player": jpype.JClass("sim.engine.entity.character.player.Player"),
        "ProtectionPrayer": jpype.JClass("HeadlessProtectionPrayer"),
        "Tile": jpype.JClass("sim.types.Tile"),
    }
...
def _invoke_jvm_getter(instance: Any, *names: str) -> Any:
    for name in names:
    ...
    for name in names:
        ...
        for candidate_name in dir(instance):
            if candidate_name == base_name or candidate_name.startswith(f"{base_name}-"):
                getter = getattr(instance, candidate_name, None)
                if getter is not None:
                    return getter()
    raise AttributeError(
        f"None of the JVM getter variants exist on {type(instance)!r}: {names!r}"
    )
