...
class Aggie : Script {
    ...
    suspend fun Player.yellowDye(target: NPC) {
        choice {
    ...
    suspend fun Player.redDye(target: NPC) {
        choice {
    ...
    suspend fun Player.blueDye(target: NPC) {
        choice {
    ...
    suspend fun Player.makeYellow(target: NPC) {
        if (!inventory.contains("coins", 5)) {
            statement("You don't have enough coins to pay for the dye.")
        } else if (!inventory.contains("onion", 2)) {
    ...
    suspend fun Player.makeRed(target: NPC) {
        if (!inventory.contains("coins", 5)) {
            statement("You don't have enough coins to pay for the dye.")
        } else if (!inventory.contains("redberries", 3)) {
    ...
    suspend fun Player.makeBlue(target: NPC) {
        if (!inventory.contains("coins", 5)) {
            statement("You don't have enough coins to pay for the dye.")
        } else if (!inventory.contains("woad_leaf", 2)) {
