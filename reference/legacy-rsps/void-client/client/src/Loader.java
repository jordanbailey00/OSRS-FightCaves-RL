...
public class Loader extends Applet {
    ...
    public static boolean hideUntilInGame = false;
    public static boolean bootstrapVisibilityExplicit = false;
    ...
    public static void main(String[] args) {
        for (int i = 0; i < args.length; i++) {
            ...
            switch (arg) {
                ...
                case "--port":
                    ...
                    break;
                case "--username":
                    autoLoginUsername = args[++i];
                    break;
                case "--password":
                    autoLoginPassword = args[++i];
                    break;
                case "--show-during-bootstrap":
                    bootstrapVisibilityExplicit = true;
                    hideUntilInGame = false;
                    break;
                case "--hide-until-ingame":
                    bootstrapVisibilityExplicit = true;
        ...
        if (!bootstrapVisibilityExplicit && !autoLoginUsername.isEmpty() && !autoLoginPassword.isEmpty()) {
            hideUntilInGame = true;
        }
        FightCavesDemoAutoLogin.configure(autoLoginUsername, autoLoginPassword, hideUntilInGame);
        Loader l = new Loader();
        l.doFrame();
    }
    ...
    void openFrame() {
        ...
        FightCavesDemoAutoLogin.attachFrame(aJFrame2);
        if (hideUntilInGame) {
            FightCavesDemoBootstrapWindow.showWindow();
        }
        aJFrame2.setVisible(!hideUntilInGame);
    }
    ...
    void startClient() {
        try {
            ...
            var_client.init();
            var_client.start();
            if (FightCavesDemoAutoLogin.isEnabled()) {
                System.out.println("FIGHT_CAVES_DEMO_AUTOLOGIN_ENABLED address=" + address + " port=" + port);
                FightCavesDemoAutoLogin.start();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
