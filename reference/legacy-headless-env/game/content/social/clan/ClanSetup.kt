...
import content.social.friend.FriendsList
...
import sim.network.login.protocol.encode.leaveClanChat
import sim.network.login.protocol.encode.updateClanChat
        ...
        interfaceOption(id = "clan_chat_setup:enter") {
            ...
            for (member in clan.members) {
                if (!clan.hasRank(member, rank)) {
                    FriendsList.leaveClan(member, true)
                }
            }
        }
