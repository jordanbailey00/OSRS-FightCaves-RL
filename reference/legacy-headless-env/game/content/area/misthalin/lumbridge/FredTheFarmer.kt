...
class FredTheFarmer : Script {
    ...
    init {
        ...
        itemOnNPCOperate("black_wool", "fred_the_farmer_lumbridge") {
            if (quest("sheep_shearer_miniquest") == "started") {
                player<Idle>("I've got some wool. I've not managed to make it into a ball, though.")
                npc<Idle>("Well, go find a spinning wheel then. You can find one on the first floor of Lumbridge Castle; just turn right and follow the path when leaving my house and you'll find Lumbridge.")
            }
        }
        itemOnNPCOperate("wool", "fred_the_farmer_lumbridge") {
            if (quest("sheep_shearer_miniquest") == "started") {
                npc<Idle>("That wool is white, I'm dealing with a guy looking to do business with black wool only.")
            }
        }
    }
    ...
    suspend fun Player.jobNotStarted() {
        choice {
            option<Idle>("I'm looking for something to kill.") {
                npc<Angry>("What, on my land? Leave my livestock alone!")
            }
            option<Disheartened>("I'm lost.") {
                npc<Quiz>("How can you be lost? Just follow the road east and south. You'll end up in Lumbridge fairly quickly.")
            }
            option<Idle>("I'm looking for work.") {
                npc<Happy>("Oh? Well, I could do with a bit of help, since you're offering.")
                npc<Idle>("I need to collect some black wool from my sheep and I'd be much obliged if you could shear them for me. While you're at it, spin the wool into balls for me too.")
                player<Quiz>("Does it have to be black wool?")
                npc<Idle>("Has to be. I'm doing business with some guy after black clothing - something to do with black looking 'cool'.")
                choice {
                    option<Idle>("It takes all sorts, I suppose.") {
                        npc<Idle>("Indeed. So if you bring me twenty balls of black wool, I'm sure I could sort out some sort of payment.")
                        player<Quiz>("So is this a quest?")
                        npc<Bored>("No, it isn't. It's work. You do what I say, then you get paid.")
                    }
                    option<Idle>("Black clothing cool? I'm not sure that's true.") {
                        npc<Idle>("That's what I thought, but I'm certainly not going to turn down the business. So if you bring me twenty balls of black wool, I'm sure I could sort out some sort of payment.")
                        player<Quiz>("So is this a quest?")
                        npc<Bored>("No, it isn't. It's work. You do what I say, then you get paid.")
                    }
                }
                choice {
                    option<Bored>("That doesn't sound very exciting.") {
    ...
    fun ChoiceOption.alreadyKnowShearing(): Unit = option<Happy>("Of course!") {
        npc<Idle>("And you know how to spin wool into balls?")
        choice {
            woolExpert()
            spinWool()
        }
    ...
    fun ChoiceOption.spinWool(): Unit = option<Idle>("I don't know how to spin wool, sorry.") {
        npc<Idle>("Don't worry it's quite simple!")
        npc<Idle>("The nearest Spinning Wheel can be found on the first floor of Lumbridge Castle.")
        npc<Idle>("To get to Lumbridge Castle just follow the road east.")
    ...
    fun ChoiceOption.woolExpert(): Unit = option<Happy>("I'm something of an expert, actually.") {
        npc<Idle>("Well, you can stop grinning and get to work then.")
        npc<Angry>("I'm not paying you by the hour!")
    }

    fun ChoiceOption.shearingTutorial(): Unit = option<Idle>("Actually, no, I don't.") {
        if (inventory.contains("shears")) {
            ...
            player<Idle>("That's all I have to do?")
            npc<Idle>("Well, once you've collected some wool you'll need to spin it into balls.")
            ...
            npc<Idle>("Well, first things first, you need some shears. There's a pair in my house on the table.")
            npc<Idle>("Or you could buy your own pair from Lumbridge General Store or from the Grand Exchange in Varrock.")
            npc<Idle>("To get to the store, turn right when leaving the farm then follow the path. The store will be just down the road on your right.")
            item("general_store_icon", 1186, "General stores are marked on the map by this symbol.")
            npc<Idle>("Once you have some shears, use them on the sheep in my field.")
            player<Happy>("Sounds easy.")
            npc<Laugh>("That's what they all say! Some of the sheep don't like it and will run away from you. Persistence is the key.")
            npc<Scared>("And watch out for rams! They will attack you if you try to shear them. You can tell them apart from the rest of the sheep by their curly horns.")
            npc<Idle>("Once you've collected some wool you can spin it into balls.")
            npc<Quiz>("Do you know how to spin wool?")
            choice {
                spinWool()
                woolExpert()
    ...
    suspend fun Player.takeJob() {
        ...
        npc<Idle>("Good. Hopefully, you'll be safe from 'The Thing'! Do you actually know how to shear a sheep?")
        choice {
            alreadyKnowShearing()
            shearingTutorial()
            option<Quiz>("What do you mean, 'The Thing'?") {
                ...
                npc<Scared>("Some say it's a black hearted shapeshifter, hungering for the souls of hard working decent folk like me. Others say it's just a sheep.")
        ...
        choice {
            if (inventory.contains("ball_of_black_wool")) {
                option<Happy>("I have some balls of black wool for you.") {
                    npc<Idle>("Give 'em here then.")
                    giveBlackWool()
                }
            } else {
                if (inventory.contains("black_wool")) {
                    option<Idle>("How many more balls of wool do you need?") {
                        npc<Idle>("You need to collect $giveBlackBallsOfWool more balls of wool")
                        player<Idle>("I've got some wool. I've not managed to make it into a ball, though.")
                        npc<Idle>("Well, go find a spinning wheel then. You can find one on the first floor of Lumbridge Castle; just turn right and follow the path when leaving my house and you'll find Lumbridge.")
                    }
                } else {
                    if (!inventory.contains("ball_of_black_wool")) {
                        option<Idle>("How many more balls of wool do you need?") {
                            npc<Idle>("You need to collect $giveBlackBallsOfWool more balls of wool")
                            player<Disheartened>("I haven't got any at the moment.")
                            npc<Happy>("Ah well at least you haven't been eaten.")
                        }
                    }
                }
            }
            option<Idle>("Can you remind me how to get balls of wool?") {
                npc<Idle>("Sure. You need to shear sheep and then spin the wool on a spinning wheel. Anything else?")
                choice {
                    remindMeShearing()
                    remindMeSpinning()
    ...
    fun ChoiceOption.theThing(): Unit = option<Amazed>("Fred! Fred! I've seen 'The Thing!'") {
        npc<Scared>("You...you actually saw it?")
        npc<Scared>("Run for the hills! $name, grab as many chickens as you can! We have to...")
        player<Amazed>("Fred!")
        npc<Scared>("..flee! Oh, woe is me! The shape-shifter is coming! We're all...")
        player<Angry>("FRED!")
        npc<Confused>("..doomed. What?")
        player<Idle>("It's not a shape-shifter or any other kind of monster.")
        npc<Quiz>("Well, what is it, boy?")
        player<Confused>("Well, it's just two penguins...disguised as a sheep.")
        npc<Confused>("...")
        npc<Amazed>("Have you been out in the sun too long?")
    }

    fun ChoiceOption.remindMeShearing(): Unit = option<Idle>("Can you tell me how to shear sheep?") {
        if (inventory.contains("shears")) {
            npc<Happy>("Well, you're halfway there already! You have a set of shears in your inventory. Just use those on a Sheep to shear it.")
            player<Idle>("That's all I have to do?")
            npc<Idle>("Well, once you've collected some wool you'll need to spin it into balls.")
            npc<Idle>("Anything else?")
            choice {
                remindMeSpinning()
                option<Happy>("That's all, thanks.")
            }
    ...
    fun ChoiceOption.remindMeSpinning(): Unit = option<Idle>("Can you tell me how to spin wool?") {
        npc<Idle>("Don't worry it's quite simple!")
        npc<Idle>("The nearest Spinning Wheel can be found on the first floor of Lumbridge Castle.")
        npc<Idle>("To get to Lumbridge Castle just follow the road east.")
        item("spinning_wheel_icon", 1200, "This icon denotes a Spinning Wheel on the world map.")
    ...
    suspend fun Player.giveBlackWool() {
        ...
        if (giveBlackBallsOfWool <= 0) {
            ...
            player<Idle>("That's all I've got so far.")
            npc<Idle>("I need $giveBlackBallsOfWool more before I can pay you.")
            player<Idle>("Okay, I'll work on it.")
        }
    }
    ...
    suspend fun Player.secondReward() {
        ...
        if (inventory.add("coins", 1940)) {
            ...
            player<Idle>("Great news, but I need to free up some inventory space first. I'll be right back.")
