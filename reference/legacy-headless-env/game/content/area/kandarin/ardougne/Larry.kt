...
//                option("Cold War") {}
    ...
    private suspend fun Player.hideAndSeek() {
        ...
        if (!ownsItem("spy_notebook")) {
            npc<Scared>("Wait! Where is your notebook? Did it fall into enemy hands?")
        ...
        npc<Scared>("Do you have news? Have you found more?")
        choice("Choose an option:") {
            option<Neutral>("I've found more penguins.") {
                npc<Scared>("More? They're spreading so quickly.")
                player<Happy>("I've found ${get("penguins_found_weekly", 0)} penguins this week.")
                choice("Choose an option:") {
                    havingTrouble()
                    claimReward()
                    option<Quiz>("What do I need to do again?") {
                        npc<Neutral>("Weren't you listening the first time? Fine, I'll explain it again, but pay attention this time.")
                        ...
                        if (!questCompleted("lunar_diplomacy")) {
                            npc<Neutral>("One last thing. If you ever discover a place called Lunar Isle, you might gain access to a spell that will help you get in touch with me from anywhere in the world. I can't say any more that that, there may be spies nearby.")
                        }
                    }
                    option<Neutral>("Never mind.") {
                        npc<Sad>("FINE. Be that way.")
                    }
                }
            }
            havingTrouble()
    ...
    private suspend fun Player.reward(chuck: Boolean = false) {
        ...
        if (points <= 0) {
            npc<Neutral>("You've found a lot of spies. But, you have no penguin points saved up. Keep looking!")
            return
        }
        if (chuck) {
            npc<Neutral>("My, you have been working hard... You have $points Penguin Points.")
        } else {
            npc<Neutral>("Well, you need rewarding for your hard work. You have $points Penguin ${"Point".plural(points)}.")
        }
        npc<Neutral>("I can either reward you with coins or experience. Which would you prefer?")
        choice {
    ...
    private suspend fun Player.explain() {
        ...
        npc<Scared>("What do you want?")
        player<Confused>("Uh, I just wanted to as-")
        npc<Angry>("SHHHHHH! They're listening. Keep your voice down.")
        player<Neutral>("*whispers* Who's listening?")
        ...
        player<Confused>("You think they're organised? They're just penguins!")
        ...
        info()
        player<Confused>("How much experience?")
        npc<Neutral>("The more penguins you find, the more experience. Are you in?")
    ...
    private suspend fun Player.info() {
        ...
        player<Confused>("What should I do after I've spied on them?")
