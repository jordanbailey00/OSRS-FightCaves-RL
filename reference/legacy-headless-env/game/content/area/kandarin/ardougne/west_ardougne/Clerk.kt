...
class Clerk : Script {
    ...
    suspend fun Player.menu() {
        npc<Idle>("Hello, welcome to the Civic Office of West Ardougne. How can I help you?")
        choice {
            if (quest("plague_city") == "need_clearance") {
                option<Idle>("I need permission to enter a plague house.") {
                    permission()
                }
            }
    ...
    suspend fun Player.throughThatDoor() {
        npc<Idle>("The city warder Bravek is in there.")
        player<Quiz>("Can I go in?")
        if (stages.contains(quest("plague_city"))) {
            npc<Idle>("I suppose so.")
        } else {
    ...
    suspend fun Player.urgent() {
        npc<Idle>("I'll see what I can do I suppose.")
        npc<Idle>("Mr Bravek, there's a man here who really needs to speak to you.")
        set("plague_city", "talk_to_bravek")
        npc<Confused>("bravek", "I suppose they can come in then. If they keep it short.")
    }

    suspend fun Player.talkToBravek() {
        npc<Idle>("Bravek will see you now but keep it short!")
