...
class Jethick : Script {
    ...
    init {
        npcOperate("Talk-to", "jethick") {
            when (quest("plague_city")) {
                ...
                "spoken_to_jethick" -> {
                    npc<Idle>("Hello. We don't get many newcomers around here.")
                    looking()
                }
                else -> npc<Idle>("Hello. We don't get many newcomers around here.")
            }
        }

        itemOnNPCOperate("*", "jethick") {
            if (it.item.id == "picture_plague_city") {
                player<Neutral>("Hi, I'm looking for a woman from East Ardougne.")
                showPicture()
            } else {
    ...
    suspend fun Player.grillOpen() {
        if (get("plaguecity_picture_asked", false)) {
            ...
            npc<Idle>("Hello, I don't recognise you. We don't get many newcomers around here.")
            player<Quiz>("How come?")
            npc<Idle>("The plague of course. Not many people want to come to a place like this and the few that do normally get stopped by the mourners.")
            npc<Idle>("All you'll find here now are the dead and the dying. Even our own king has abandoned us.")
            player<Quiz>("Your king?")
            npc<Idle>("Yes, King Tyras of West Ardougne. He's the brother of King Lathas, the ruler of East Ardougne.")
    ...
    private suspend fun Player.looking() {
        ...
        npc<Confused>("East Ardougnian women are easier to find in East Ardougne. Not many would come to West Ardougne to find one. Although the name is familiar, what does she look like?")
        if (inventory.contains("picture_plague_city")) {
            showPicture()
        } else {
            player<Confused>("Um... brown hair... in her twenties...")
            if (!get("plaguecity_picture_asked", false)) {
                set("plaguecity_picture_asked", true)
            }
            npc<Confused>("Hmmm, that doesn't narrow it down a huge amount... I'll need to know more than that, or see a picture?")
        }
    }
    ...
    suspend fun Player.spokenToJethick() {
        npc<Idle>("They live in the small timbered building at the far north side of town. I've not seen her around here in a while, mind.")
        if (!ownsItem("book_turnip_growing_for_beginners")) {
            npc<Idle>("I don't suppose you could run me a little errand while you're over there? I borrowed this book from them, can you return it?")
            choice {
